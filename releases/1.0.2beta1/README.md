# CodeBridge 1.0.2beta1

| 文件 | 平台 | 类型 | 说明 |
|---|---|---|---|
| CodeBridge-1.0.2beta1-android.apk | Android | APK | 手机端（Kotlin + Compose），正式密钥签名 |
| CodeBridge-1.0.2beta1-windows-installer.exe | Windows x64 | 安装包 | NSIS 安装程序 |
| CodeBridge-1.0.2beta1-windows-x64-portable.exe | Windows x64 | 便携版 | 免安装，双击运行 |
...
- 手机端：新增二维码扫码配对（扫描 PC 端二维码自动填入地址 / 端口 / Token）
- PC 端：验证码自动输入到当前焦点输入框（Windows / macOS / Linux）
- PC 端：验证码统计面板（今日 / 累计数量 + 来源应用分布）
- PC 端：系统通知、历史记录搜索与自动清理
- PC 端：自动更新检查、主题定制、多语言界面
